import { useContext, useState } from "react";
import { ThemeContext } from "../context/ThemeContext";
import { UserContext } from "../context/UserContext";
import Tooltip, { tooltipClasses } from "@mui/material/Tooltip";
import { styled } from "@mui/material/styles";
function TwitterLikeAnimation() {
  const BootstrapTooltip = styled(({ className, ...props }) => (
    <Tooltip {...props} arrow classes={{ popper: className }} />
  ))(({ theme }) => ({
    [`& .${tooltipClasses.arrow}`]: {
      color: "transparent",
    },
    [`& .${tooltipClasses.tooltip}`]: {
      backgroundColor: themeName === "dark-theme" ? "#495a68" : "",
    },
  }));
  const [{ theme, themeName }] = useContext(ThemeContext);
  const [liked, setLiked] = useState(false);
  const { userInfo } = useContext(UserContext);

  const [likesCount, setLikesCount] = useState(null);

  const handleLike = () => {
    setLiked(true);
    setLikesCount((prevCount) => prevCount + 1);
  };

  const handleUnlike = () => {
    setLiked(false);
    setLikesCount((prevCount) => prevCount - 1);
  };

  const [likeIconHovered, setLikeIconHovered] = useState(null);
  return (
    // <Tooltip title={liked ? `Unlike` : "Like"}>
    //   <div
    //     onClick={() => handleLike(!liked)}
    //     style={{
    //       position: "relative",
    //       width: "34px",
    //       height: "34px",
    //       borderRadius: "50%",
    //       display: "flex",
    //       justifyContent: "center",
    //       alignItems: "center",
    //       backgroundColor:
    //         likeIconHovered && themeName !== "dark-theme"
    //           ? "#f9e3eb"
    //           : likeIconHovered && themeName === "dark-theme"
    //           ? "#4f102b"
    //           : null,
    //     }}
    //     onMouseEnter={() => setLikeIconHovered(true)}
    //     onMouseLeave={() => setLikeIconHovered(false)}
    //   >
    //     <div
    //       style={{
    //         position: "absolute",
    //         width: "60px",
    //         height: "34px",
    //       }}
    //       onClick={liked ? handleLike : handleUnlike}
    //       className={liked ? `is_animating heart` : `heart`}
    //     ></div>
    //   </div>
    // </Tooltip><div className="twitter-like-icon-test">

    <div className="twitter-like-icon-test">
      <input id="toggle-heart" type="checkbox" />
      <label htmlFor="toggle-heart">
        {" "}
        <svg
          onClick={liked ? handleUnlike : handleLike}
          fill={liked ? "rgb(249, 24, 128)" : null}
          width={`${1.25}em`}
          height={`${1.25}em`}
          viewBox="0 0 24 24"
          aria-hidden="true"
          className="svg-heart r-4qtqp9 r-yyyyoo r-dnmrzs r-bnwqim r-1plcrui r-lrvibr r-1xvli5t r-1hdv0qi"
        >
          {liked ? (
            <g>
              <path
                stroke="black"
                strokeWidth="0.2"
                d="M20.884 13.19c-1.351 2.48-4.001 5.12-8.379 7.67l-.503.3-.504-.3c-4.379-2.55-7.029-5.19-8.382-7.67-1.36-2.5-1.41-4.86-.514-6.67.887-1.79 2.647-2.91 4.601-3.01 1.651-.09 3.368.56 4.798 2.01 1.429-1.45 3.146-2.1 4.796-2.01 1.954.1 3.714 1.22 4.601 3.01.896 1.81.846 4.17-.514 6.67z"
              ></path>
            </g>
          ) : (
            <g>
              <path d="M16.697 5.5c-1.222-.06-2.679.51-3.89 2.16l-.805 1.09-.806-1.09C9.984 6.01 8.526 5.44 7.304 5.5c-1.243.07-2.349.78-2.91 1.91-.552 1.12-.633 2.78.479 4.82 1.074 1.97 3.257 4.27 7.129 6.61 3.87-2.34 6.052-4.64 7.126-6.61 1.111-2.04 1.03-3.7.477-4.82-.561-1.13-1.666-1.84-2.908-1.91zm4.187 7.69c-1.351 2.48-4.001 5.12-8.379 7.67l-.503.3-.504-.3c-4.379-2.55-7.029-5.19-8.382-7.67-1.36-2.5-1.41-4.86-.514-6.67.887-1.79 2.647-2.91 4.601-3.01 1.651-.09 3.368.56 4.798 2.01 1.429-1.45 3.146-2.1 4.796-2.01 1.954.1 3.714 1.22 4.601 3.01.896 1.81.846 4.17-.514 6.67z"></path>
            </g>
          )}
        </svg>
      </label>{" "}
    </div>
  );
}

export default TwitterLikeAnimation;
