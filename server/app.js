...
// socket variables start to check
...
// socket variables finish to check
...
io.on("connection", async (socket) => {
  ...
  setTimeout(() => {
    ...
    if (currentUrlForSocketId?.startsWith("/messages/")) {
      ...
      console.log("Chat detail page route is not active.----------------- ");
      ...
      User.findById(userId)
        ...
        .then((user) => {
          ...
          socket.on("join_spesific_message_room", (data) => {
            ...
            const activeRoomWithUsers = interactedChatRooms.find((eachRoom) => {
            ...
            setTimeout(() => {
              if (!activeRoomWithUsers) {
                ...
                if (
                  interactedChatRooms[indexOfThisRoom].room.activeUsers[0].user1
                    .username === null
                ) {
                  ...
                  interactedChatRooms[indexOfThisRoom].room.activeUsers[0].user2
                    .username === null
                ) {
                  interactedChatRooms[indexOfThisRoom].room.roomName = room;
                  interactedChatRooms[
  ...
  socket.on("join_user_room", async (data) => {
    ...
    const room = [activeUser.username, selectedUser.username].sort().join("_");
  ...
  socket.on("disconnect", () => {
    ...
    if (findedUserIndex !== -1) {
      ...
      console.log("User not found to disconnect with username and socket id ");
    }
  });
});
// socket finish to check
